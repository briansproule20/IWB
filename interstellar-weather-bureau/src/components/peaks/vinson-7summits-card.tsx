...
export default function Vinson7SummitsCard() {
  const [weather, setWeather] = useState<any>(null);
  const heightM = 4892;
  const heightFt = 16050;
  const continent = "Antarctica";

  useEffect(() => {
    fetch('/api/weather/vinson')
      .then(res => res.json())
  ...
  return (
    <div className="flex h-full flex-col space-y-2">
      <div>
        <h3 className="text-lg font-bold text-gray-900 dark:text-white">Vinson Massif</h3>
      ...
      <div className="flex-1 space-y-2">
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            ...
            <div className="text-xs text-gray-600 dark:text-gray-400">Sixth highest</div>
          </div>
        </div>
      ...
      <div className="text-xs text-gray-400">78°31′S 85°37′W</div>
    </div>
  );
}
