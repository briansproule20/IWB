...
export default function Kilimanjaro7SummitsCard() {
  const [weather, setWeather] = useState<any>(null);
  const heightM = 5895;
  const heightFt = 19341;
  const continent = "Africa";

  useEffect(() => {
    fetch('/api/weather/kilimanjaro')
      .then(res => res.json())
  ...
  return (
    <div className="flex h-full flex-col space-y-2">
      <div>
        <h3 className="text-lg font-bold text-gray-900 dark:text-white">Kilimanjaro</h3>
        <p className="text-xs text-gray-500">{continent} • Tanzania</p>
      </div>
      <div className="flex-1 space-y-2">
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            ...
            <div className="text-xs text-gray-600 dark:text-gray-400">Fourth highest</div>
          </div>
        </div>
      ...
      <div className="text-xs text-gray-400">3°04′S 37°21′E</div>
    </div>
  );
}
