...
export default function Denali7SummitsCard() {
  const [weather, setWeather] = useState<any>(null);
  const heightM = 6190;
  const heightFt = 20310;
  const continent = "North America";

  useEffect(() => {
    fetch('/api/weather/denali')
      .then(res => res.json())
  ...
  return (
    <div className="flex h-full flex-col space-y-2">
      <div>
        <h3 className="text-lg font-bold text-gray-900 dark:text-white">Denali</h3>
        <p className="text-xs text-gray-500">Formerly Mt. McKinley</p>
        <p className="text-xs text-gray-500">{continent} • USA (Alaska)</p>
      </div>
      <div className="flex-1 space-y-2">
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            ...
            <div className="text-xs text-gray-600 dark:text-gray-400">Third highest</div>
          </div>
        </div>
        <div className="space-y-2">
          <div className="rounded-lg bg-purple-50 p-2 dark:bg-purple-900/20">
            <div className="text-xs text-purple-600 dark:text-purple-400">Continent</div>
            <div className="text-lg font-bold text-purple-900 dark:text-purple-200">{continent} - Highest Peak</div>
          </div>
          <div className="space-y-2">
      ...
      <div className="text-xs text-gray-400">63°04′N 151°00′W</div>
    </div>
  );
}
