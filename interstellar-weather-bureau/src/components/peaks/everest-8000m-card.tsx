...
export default function Everest8000mCard() {
  const [weather, setWeather] = useState<any>(null);
  const heightM = 8849;
  const heightFt = 29032;
  const prominence = 8849;

  useEffect(() => {
    fetch('/api/weather/everest')
      .then(res => res.json())
  ...
  return (
    <div className="flex h-full flex-col space-y-2">
      <div>
        <h3 className="text-lg font-bold text-gray-900 dark:text-white">Mount Everest</h3>
      ...
      <div className="flex-1 space-y-2">
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            ...
            <div className="text-xs text-gray-600 dark:text-gray-400">Highest on Earth</div>
          </div>
        </div>
        <div className="space-y-2">
          <div className="rounded-lg bg-blue-50 p-2 dark:bg-blue-900/20">
            <div className="text-xs text-blue-600 dark:text-blue-400">Rank</div>
            <div className="text-lg font-bold text-blue-900 dark:text-blue-200">#1 of 14 Eight-thousanders</div>
          </div>
      ...
      <div className="text-xs text-gray-400">27°59′N 86°55′E</div>
    </div>
  );
}
