...
export default function Elbrus7SummitsCard() {
  const [weather, setWeather] = useState<any>(null);
  const heightM = 5642;
  const heightFt = 18510;
  const continent = "Europe";

  useEffect(() => {
    fetch('/api/weather/elbrus')
      .then(res => res.json())
  ...
  return (
    <div className="flex h-full flex-col space-y-2">
      <div>
        <h3 className="text-lg font-bold text-gray-900 dark:text-white">Mount Elbrus</h3>
        <p className="text-xs text-gray-500">{continent} • Russia</p>
      </div>
      <div className="flex-1 space-y-2">
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            ...
            <div className="text-xs text-gray-600 dark:text-gray-400">Fifth highest</div>
          </div>
        </div>
      ...
      <div className="text-xs text-gray-400">43°21′N 42°26′E</div>
    </div>
  );
}
