...
export default function NangaParbat8000mCard() {
  const [weather, setWeather] = useState<any>(null);
  const heightM = 8126;
  const heightFt = 26660;
  const prominence = 4608;

  useEffect(() => {
    fetch('/api/weather/nanga-parbat')
      .then(res => res.json())
  ...
  return (
    <div className="flex h-full flex-col space-y-2">
      <div>
        <h3 className="text-lg font-bold text-gray-900 dark:text-white">Nanga Parbat</h3>
        <p className="text-xs text-gray-500">Himalayas • Pakistan</p>
      </div>
      <div className="flex-1 space-y-2">
        ...
        <div className="space-y-2">
          <div className="rounded-lg bg-blue-50 p-2 dark:bg-blue-900/20">
            ...
            <div className="text-lg font-bold text-blue-900 dark:text-blue-200">#9 of 14 Eight-thousanders</div>
          </div>
          <div className="space-y-2">
      ...
      <div className="text-xs text-gray-400">35°14′N 74°36′E</div>
    </div>
  );
}
