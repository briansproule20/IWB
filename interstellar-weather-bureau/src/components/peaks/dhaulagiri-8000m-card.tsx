...
export default function Dhaulagiri8000mCard() {
  const [weather, setWeather] = useState<any>(null);
  const heightM = 8167;
  const heightFt = 26795;
  const prominence = 3357;

  useEffect(() => {
    fetch('/api/weather/dhaulagiri')
      .then(res => res.json())
  ...
  return (
    <div className="flex h-full flex-col space-y-2">
      <div>
        <h3 className="text-lg font-bold text-gray-900 dark:text-white">Dhaulagiri</h3>
        <p className="text-xs text-gray-500">Himalayas • Nepal</p>
      </div>
      <div className="flex-1 space-y-2">
        ...
        <div className="space-y-2">
          <div className="rounded-lg bg-blue-50 p-2 dark:bg-blue-900/20">
            ...
            <div className="text-lg font-bold text-blue-900 dark:text-blue-200">#7 of 14 Eight-thousanders</div>
          </div>
          <div className="space-y-2">
      ...
      <div className="text-xs text-gray-400">28°42′N 83°29′E</div>
    </div>
  );
}
