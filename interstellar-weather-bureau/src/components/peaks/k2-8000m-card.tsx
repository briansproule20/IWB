...
export default function K28000mCard() {
  const [weather, setWeather] = useState<any>(null);
  const heightM = 8611;
  const heightFt = 28251;
  const prominence = 4017;

  useEffect(() => {
    fetch('/api/weather/k2')
      .then(res => res.json())
  ...
  return (
    <div className="flex h-full flex-col space-y-2">
      <div>
        <h3 className="text-lg font-bold text-gray-900 dark:text-white">K2</h3>
        <p className="text-xs text-gray-500">Karakoram • Pakistan/China</p>
      </div>
      <div className="flex-1 space-y-2">
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            ...
            <div className="text-xs text-gray-600 dark:text-gray-400">22nd most prominent</div>
          </div>
        </div>
        <div className="space-y-2">
          <div className="rounded-lg bg-blue-50 p-2 dark:bg-blue-900/20">
            <div className="text-xs text-blue-600 dark:text-blue-400">Rank</div>
            <div className="text-lg font-bold text-blue-900 dark:text-blue-200">#2 of 14 Eight-thousanders</div>
          </div>
      ...
      <div className="text-xs text-gray-400">35°53′N 76°31′E</div>
    </div>
  );
}
