...
export default function Aconcagua7SummitsCard() {
  const [weather, setWeather] = useState<any>(null);
  const heightM = 6961;
  const heightFt = 22838;
  const continent = "South America";

  useEffect(() => {
    fetch('/api/weather/aconcagua')
      .then(res => res.json())
  ...
  return (
    <div className="flex h-full flex-col space-y-2">
      <div>
        <h3 className="text-lg font-bold text-gray-900 dark:text-white">Aconcagua</h3>
        <p className="text-xs text-gray-500">{continent} • Argentina</p>
      </div>
      <div className="flex-1 space-y-2">
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            ...
            <div className="text-xs text-gray-600 dark:text-gray-400">Second highest</div>
          </div>
        </div>
      ...
      <div className="text-xs text-gray-400">32°39′S 70°01′W</div>
    </div>
  );
}
