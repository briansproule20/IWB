...
// Lake Superior coordinates (Edmund Fitzgerald wreck site area)
const LAKE_SUPERIOR_LAT = 46.99;
const LAKE_SUPERIOR_LON = -85.11;

export async function GET() {
  try {
    // Fetch atmospheric weather data
    const weatherParams = new URLSearchParams({
      latitude: LAKE_SUPERIOR_LAT.toString(),
      longitude: LAKE_SUPERIOR_LON.toString(),
      current: [
        'temperature_2m',
    ...
    return NextResponse.json({
      location: {
        name: 'Lake Superior',
        latitude: LAKE_SUPERIOR_LAT,
        longitude: LAKE_SUPERIOR_LON,
      },
      weather: weatherData,
      marine: marineData,
    ...
    console.error('Lake Superior weather API error:', error);
    return NextResponse.json(
      {
