...
// Drake Passage coordinates
const DRAKE_PASSAGE_LAT = -58.0;
const DRAKE_PASSAGE_LON = -65.0;

export async function GET() {
  try {
    // Fetch atmospheric weather data
    const weatherParams = new URLSearchParams({
      latitude: DRAKE_PASSAGE_LAT.toString(),
      longitude: DRAKE_PASSAGE_LON.toString(),
      current: [
        'temperature_2m',
    ...
    return NextResponse.json({
      location: {
        name: 'Drake Passage',
        latitude: DRAKE_PASSAGE_LAT,
        longitude: DRAKE_PASSAGE_LON,
      },
      weather: weatherData,
      marine: marineData,
    ...
    console.error('Drake Passage weather API error:', error);
    return NextResponse.json(
      {
