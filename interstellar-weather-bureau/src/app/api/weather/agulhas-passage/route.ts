...
// Agulhas Passage coordinates (Cape Agulhas area)
const AGULHAS_PASSAGE_LAT = -35.0;
const AGULHAS_PASSAGE_LON = 20.0;

export async function GET() {
  try {
    // Fetch atmospheric weather data
    const weatherParams = new URLSearchParams({
      latitude: AGULHAS_PASSAGE_LAT.toString(),
      longitude: AGULHAS_PASSAGE_LON.toString(),
      current: [
        'temperature_2m',
    ...
    return NextResponse.json({
      location: {
        name: 'Agulhas Passage',
        latitude: AGULHAS_PASSAGE_LAT,
        longitude: AGULHAS_PASSAGE_LON,
      },
      weather: weatherData,
      marine: marineData,
    ...
    console.error('Agulhas Passage weather API error:', error);
    return NextResponse.json(
      {
